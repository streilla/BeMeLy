...
# Training settings
parser = argparse.ArgumentParser(description='Training multi-class ABMIL models')
...
def compute_eval_metrics(model, loader, n_classes):
    all_outputs, all_labels, all_predicted, all_slides = [], [], [], []
    
    for features, labels, slide_id in loader:      
        features, labels = {'features': features.to(device)}, labels.to(device)
        outputs = model(features)
        ...
        all_slides.append(np.array(slide_id))
    ...
    if n_classes > 2:
        ...
        all_outputs = [k[i] for k,i in zip(all_outputs, all_labels)]
    ...
    cols = ['slide_id', 'label'] + [f'pred_{k}' for k in range(n_classes)]
    out_df = pd.DataFrame(np.concatenate((np.expand_dims(all_slides, 1), np.expand_dims(all_labels, 1), all_outputs), axis=1), columns=cols)
    
    return (auc, accuracy, balanced_accuracy, f1, recall, precision), cm, out_df
...
if __name__ == '__main__':
    ...
    for model_name in model_names:
        ...
        for n_split in range(n_splits):
            ...
            model = MultiClassificationModel(input_feature_dim=input_dim, dropout=0.2, num_classes=args.n_classes).to(device)
        ...
        for i, (train_loader, val_loader) in enumerate(zip(train_loaders, val_loaders)):
            ...
            model.eval()
            with torch.no_grad():
                metrics_train, cm_train, _ = compute_eval_metrics(model, train_loader, args.n_classes)
                metrics_val, cm_val, _ = compute_eval_metrics(model, val_loader, args.n_classes)
                metrics_test, cm_test, out_df_test = compute_eval_metrics(model, test_loader, args.n_classes)
                ...
                all_cms_val.append(cm_val)
                all_cms_test.append(cm_test)
                out_df_test.to_csv(os.path.join(results_dir, f'{model_name}_test_df_{i}.csv'), index=False)
                #all_outputs_test.append(out_df_test)
        ...
        all_cms_train = np.array(all_cms_train)
        all_cms_val = np.array(all_cms_val)
        all_cms_test = np.array(all_cms_test)
        ...
        np.save(os.path.join(results_dir, f'{model_name}_train_cm.npy'), all_cms_train)
        np.save(os.path.join(results_dir, f'{model_name}_val_cm.npy'), all_cms_val)
        np.save(os.path.join(results_dir, f'{model_name}_test_cm.npy'), all_cms_test)
        
        """np.save(os.path.join(results_dir, f'{model_name}_test_outputs.npy'), all_outputs_test)
        np.save(os.path.join(results_dir, f'{model_name}_test_labels.npy'), labels_test)"""